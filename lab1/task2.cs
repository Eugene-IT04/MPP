...
namespace lab1_mpp
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            if (node == null)
            {
                hashMap[hashCode0 % mapSize] = new Node(bufString, 1, pageCounter);
                hashMap[hashCode0 % mapSize].pages = $"{pageCounter}";
                difWordsCount++;
                goto end1;
            }
            else
            {
                ...
                {
                    node.count++;
                    if (node.lastPage != pageCounter)
                    {
                        node.lastPage = pageCounter;
                        node.pages += $", {pageCounter}";
                    }
                    goto end1;
                }
                ...
                if (newNode == null)
                {
                    node.nextNode = new Node(bufString, 1, pageCounter);
                    node.pages = $"{pageCounter}";
                    difWordsCount++;
                    goto end1;
                }
...
escape:
            ...
            if (wordsToCount[index5].value[0] < wordsToCount[index5 + 1].value[0])
            {
            ...
            int printIndex = 0;
            start3:
            if (result[printIndex].count < 400)
            Console.WriteLine(result[printIndex].value + " - " + result[printIndex].pages);
            printIndex++;
    ...
    class Node
    {
        public Node(String value, int count, int lastPage)
        {
            this.value = value;
            this.count = count;
